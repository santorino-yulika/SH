...
# ===========================
# Підключення email через Streamlit Secrets
# ===========================
EMAIL_ADDRESS = st.secrets["email"]["EMAIL_ADDRESS"]
EMAIL_PASSWORD = st.secrets["email"]["EMAIL_PASSWORD"]
EMAIL_RECEIVERS = [email.strip() for email in st.secrets["email"]["EMAIL_RECEIVERS"].split(",")]

# ===========================
# DEMO EMAIL (MOCK)
# ===========================
def send_email_demo(subject, body, recipients):
...
with st.form("operation_form"):
    ...
    amputation_stage = st.selectbox("Стадія ампутації", ["Первинна", "Ревізійна", "Реампутація"])
    amputation_level = st.selectbox("Рівень ампутації", ["Стегно", "Гомілка", "Плече", "Передпліччя", "Пересадка шкіри"])
    ...
    comorbidities = st.multiselect("Супутні захворювання", ["Діабет", "Анемія", "ІХС", "Коагулопатія"])

    # ---------- Блок C: Трансфузіологія ----------
    st.subheader("🔹 Трансфузіологічний блок")
    blood_loss = st.selectbox("Очікувана крововтрата", ["< 500 мл", "500–1000 мл", "> 1000 мл"])
    blood_needed = st.radio("Потреба в крові", ["Так", "Можливо", "Ні"])
    blood_components = st.multiselect("Необхідні компоненти", ["Еритроцити", "Плазма", "Тромбоцити"])
    blood_group = st.text_input("Група крові (якщо відома)")
    urgency = st.selectbox("Терміновість", ["Планово", "Терміново", "Негайно"])

    # ---------- Блок D: Інші ресурси ----------
    st.subheader("🔹 Інші ресурси")
    equipment = st.multiselect("Потрібне обладнання", ["Апаратура ШВЛ", "Моніторинг", "Рентген", "УЗД"])
...
if submitted:
    ...
    # === 1) Формуємо "рядок таблиці" (для демо) ===
    row = {
        "timestamp": timestamp,
...
"""
    ...
    if recipients:
        ...
        st.warning("⚠️ Жоден відділ не був автоматично чи вручну обраний для сповіщення.")
